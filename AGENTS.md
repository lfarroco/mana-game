...
- [x] **Server Phase 3 — client integration**: DONE (2026-08-13). `phaser/src/RemoteServer.ts` rewritten as a thin HTTP adapter for the new API: `createRemoteServer({ fetch, serverUrl, getBearerToken })` factory + `remoteServer` singleton, `Authorization: Bearer <token>` from `steamAuth.getBearerToken()` (no token → reject before fetch), `POST /api/v1/sessions` `{ crystalId, queueType }` (no client seed — server generates it), `POST /sessions/current/actions` decoding `CombatStateDto` via the core `CombatCodec`, `GET`/`DELETE /sessions/current` (404 → null / idempotent), non-2xx surfaced as `RemoteServerError` with the server's `{ error, message }` (401 → "login expired, please re-authenticate"). `GameServer.getServer()` returns the `remoteServer` singleton for multiplayer; single-player unchanged (`LocalServer`). Supabase quarantine deleted per code-quality-cleanup.md §3: `src/lib/supabase.ts`, `phaser/supabase/`, `scripts/bundle-edge.ts`, `Screens/ArenaLobby/`, `@supabase/supabase-js` + lockfile, `test:supabase`/`bundle:edge`/`deploy:functions` scripts, stale jest ignore entries. Tests: 52 phaser tests green (41 baseline + 11 new RemoteServer suite), typecheck/lint clean; server untouched (148 tests green). Deviation: `NODE_OPTIONS='--experimental-vm-modules'` breaks jest globals on this machine (`jest is not defined`), so tests ran via `jest --runInBand --passWithNoTests` (matches CI `test:ci`).
- [x] **Server Phase 4 — durable persistence**: DONE (2026-08-14). SQLite (`better-sqlite3`) repositories behind the existing interfaces (`server/src/persistence/sqlite.ts` — sessions/combat states as JSON, players/tokens/ghosts/recently-fought/ratings), `MANA_SQLITE_PATH` config (unset = in-memory default; `:memory:` supported), restart-survival test (close + reopen the DB file mid-combat → `GET /sessions/current` resumes with the combat state intact). 167 server tests green, typecheck/build clean. Schema deviation: combat state stored as a CombatCodec DTO in a separate `combat_states` table (SessionData embeds a Map-carrying live object that plain JSON can't hold).
...
- [x] **Card pool tier rebalance + designer metadata** — DONE (2026-08-14): pool is now **61 bronze / 21 silver / 10 gold** (was 61/8/23) — 13 locked golds demoted to silver (`rank 3→2`, no stat changes; all already had reactions, power ≤ 75, ≤ 3 slots). Kept as gold: the 5 unlocked chase cards (`toxicologist`, `expedition_leader`, `vanguard`, `veteran_paladin`, `webert_the_old`) + 2 multiply engines (`fate_shifter`, `destiny_balancer`, test-required) + 3 flagship type engines (`warbringer`, `aegis_archon`, `plague_sovereign`). Added optional authoring-only `description` + `tags` (closed `CARD_TAGS` vocabulary) to every non-core card — never read at runtime, never test-enforced (per design decision). New balance-test guard: silvers > golds and golds ≤ 10. Docs updated (`card-design-philosophy.md` §1/§4.1, `card-system-risks-and-roadmap.md` §1/§7). 603 core tests green (66 suites), typecheck clean; phaser typecheck/lint clean. **Deferred**: silver-identity pass on the 13 demoted cards (each should enable exactly one archetype; re-tune numbers if playtest warrants) — tracked in docs/card-system-risks-and-roadmap.md §1.
