...
describe("Full Session Flow - Server Side", () => {
	...
	async function completeRound(playerId: string): Promise<{ won: boolean; session: any }> {
		...
		// If coming from previous combat, complete pending upgrade phase first.
		let options = await manager.getPhaseOptions(playerId);
		if (
			(options.phase === "upgrade_core" || options.phase === "add_reaction_core") &&
		...
		expect(
			["encounter", "shop", "orb_shop", "upgrade_core", "add_reaction_core", "combat"].includes(
				options.phase
			)
		).toBe(true);
		...
		const safeOption =
			options.options.find((opt) => !specialEncounters.includes(opt.id)) || options.options[0];
		...
		// Step 3: Encounter (or other valid intermediary phases)
		options = await manager.getPhaseOptions(playerId);
		expect(
			[
				"encounter",
		...
		options = await manager.getPhaseOptions(playerId);
		expect(["shop", "orb_shop", "combat"].includes(options.phase)).toBe(true);
		expect(options.options.length).toBeGreaterThan(0);
		...
		// Step 5: Encounter
		options = await manager.getPhaseOptions(playerId);
		expect(["encounter", "orb_shop", "combat"].includes(options.phase)).toBe(true);
		expect(options.options.length).toBeGreaterThan(0);
	...
	describe("Basic Session Flow", () => {
		...
		it.skip("should perform combat and generate combat logs", async () => {
			...
			// Handle orb shop transition if encountered instead of combat
			...
			// Now in combat phase
			expect(["combat", "orb_shop"].includes(options.phase)).toBe(true);
			if (options.phase === "orb_shop") {
				await manager.handleAction(playerId, options.options[0].id);
				options = await manager.getPhaseOptions(playerId);
				expect(options.phase).toBe("combat");
	...
	describe("Complete Session Tests", () => {
		it("should complete a full round (all phases)", async () => {
			...
			expect(result.session.round).toBeGreaterThanOrEqual(1);
			expect(["encounter", "upgrade_core", "add_reaction_core", "victory", "game_over"]).toContain(
				result.session.phase
			);
		});

		it.skip("should play multiple rounds until victory or defeat", async () => {
			...
			while (
				...
				console.log(
					`Round ${roundsPlayed} complete - Won: ${result.won}, Wins: ${session.wins}, Losses: ${session.losses}`
				);
			}
			...
			console.log(
				`Session ended with ${session.wins} wins and ${session.losses} losses after ${roundsPlayed} rounds`
			);
		});
	...
	describe("Edge Cases", () => {
		...
		it.skip("should handle orb shop mechanics", async () => {
		...
		it.skip("should track action log throughout session", async () => {
			...
			// Verify action log tracked actions for this flow; some handlers normalize action ids.
			const loggedActions = session?.action_log.map((entry: any) => entry.actionId);
			expect(loggedActions).toContain(action1);
			expect((loggedActions || []).length).toBeGreaterThanOrEqual(2);
		});
	...
	describe("Combat System Validation", () => {
		it("should generate valid combat state with all required fields", async () => {
			...
			if (
				combatOptions.phase === "encounter" &&
				combatOptions.options[0]?.id === "combat_encounter"
			) {
				await manager.handleAction(playerId, "combat_encounter");
				combatOptions = await manager.getPhaseOptions(playerId);
			}

			expect(["combat", "orb_shop"].includes(combatOptions.phase)).toBe(true);
			if (combatOptions.phase === "combat") {
		...
		it("should have enemy team scale with round number", async () => {
			...
			if (session && session.phase === "encounter") {
				...
				if (
					round2Combat.phase === "encounter" &&
					round2Combat.options[0]?.id === "combat_encounter"
				) {
					await manager.handleAction(playerId, "combat_encounter");
				...
				if (round2Combat.phase === "combat" && round2Combat.combatState) {
					...
					const round2EnemyPower = round2Enemy.reduce(
						(sum: number, u: any) => sum + (u.power || 0),
						0
					);
					...
					console.log(
						`Round 1 enemy power: ${round1EnemyPower}, Round 2 enemy power: ${round2EnemyPower}`
					);
				}
			}
		});
