/**
 * Rating service — wins-based rating deltas on run completion.
 *
 * Ported from the retired Supabase backend
 * (`phaser/supabase/functions/_shared/multiplayer-rating.ts`, deleted in
 * Phase 3): the delta is a flat wins-tier bonus — gold 6, silver 4, bronze 2,
 * otherwise 1 — based on the run's final win count (thresholds 10/8/5).
 * Elo-style per-battle deltas are a documented future option
 * (docs/game-server.md, "Risks & open questions").
 *
