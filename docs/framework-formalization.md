...
- [x] Keep Phaser adapters in `phaser/src/`
      ...
      moduleNameMapper). `BattlegroundScreen`'s `createScreen` migration was
      completed (2026-08-01, Cline) — it keeps its handler-based phase
      orchestration alongside the framework lifecycle via a `runPhaseHandler`
      adapter.
