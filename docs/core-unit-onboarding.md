...
> Note: this fixes a latent inconsistency — today *two* cores (`mana_crystal`,
...
```ts
function generateCoreUpgradeOptions(session: Models.SessionData): Models.PhaseOption[]
```
...
- Keep enemy *cores* simple; scale enemy *stats* — this preserves the
