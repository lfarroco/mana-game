...
type FakeObj = { destroy: jest.Mock<() => void | Promise<void>> };

const fakeObj = (): FakeObj => ({ destroy: jest.fn() }) as unknown as FakeObj;

/** Fake whose destroy() delegates to a custom (possibly async) implementation. */
const asyncDestroyObj = (
...
describe("createScreen", () => {
  ...
  // -----------------------------------------------------------------------
  // P1 — async lifecycle: async Destroyable teardowns are awaited by
  // go()/refresh(), and rapid transitions serialize on a per-screen chain
  // (self-healing like the ScreenManager nav mutex).
  // -----------------------------------------------------------------------
