...
describe("Functional", () => {
  ...
  describe("Option", () => {
    ...
    describe("getOrElse", () => {
      ...
      it("returns default for None", () => {
        expect(F.getOrElse(F.none as F.Option<string>, "default")).toBe(
          "default",
        );
      });
    });
  ...
  describe("chainOption (flatMapOption)", () => {
    ...
    it("returns None when input is None", () => {
      const result = F.chainOption(F.none as F.Option<number>, (x) =>
        F.some(x * 2),
      );
      expect(result).toEqual(F.none);
    });
    ...
    it("flatMapOption is same as chainOption", () => {
      expect(F.flatMapOption(F.some(3), (x) => F.some(x + 1))).toEqual(
        F.some(4),
      );
    });
  });
