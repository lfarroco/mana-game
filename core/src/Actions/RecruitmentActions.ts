...
function getShopRecruitRank(
  ...
  const selectedOption = session.options.find(
    (option): option is Models.PhaseOption & { recruitRank?: number } =>
      option.id === cardId,
  );
...
export function recruitUnit(
  ...
  if (!card) {
    console.error(
      "recruitmentActions",
      `Card with ID ${cardId} not found for recruitment`,
    );
    return session;
  }
  ...
  if (existingUnitIndex >= 0) {
    console.debug(
      "recruitmentActions",
      `Unit with card ID ${cardId} already exists, attempting upgrade`,
    );
  ...
  if (units.length < 9) {
    ...
    if (targetPosition) {
      ...
      const isWithinBounds = x >= 0 && x <= 2 && y >= 0 && y <= 2;

      if (!isWithinBounds) {
        console.warn(
          "recruitmentActions",
          `Target position ${x},${y} is out of bounds, ignoring target slot`,
        );
        return session;
      }
      ...
      if (occupied) {
        console.warn(
          "recruitmentActions",
          `Target position ${x},${y} is already occupied, ignoring target slot`,
        );
        return session;
      }
    ...
    if (isSome(targetPos)) {
      ...
      if (recruitRank > 1) {
        console.debug(
          "recruitmentActions",
          `Recruited unit ${cardId} at Rank ${newUnit.rank}`,
        );
      }
      ...
      session.runStats.unitUsage[cardId] =
        (session.runStats.unitUsage[cardId] || 0) + 1;
  ...
  console.warn(
    "recruitmentActions",
    `Failed to recruit unit with card ID ${cardId}: team is full`,
  );
