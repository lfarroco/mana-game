...
const mockCards = [
	{ id: "unit_a", pic: "", cooldown: 1000, effects: [], reactions: [], rank: 1 },
	{ id: "unit_b", pic: "", cooldown: 1000, effects: [], reactions: [], rank: 1 },
	{ id: "unit_c", pic: "", cooldown: 1000, effects: [], reactions: [], rank: 1 },
];

function makeUnit(id: string, cardId: string, pos: [number, number], overrides: Partial<Models.Unit> = {}): Models.Unit {
	return {
		id,
		cardId,
		pic: "",
...
describe("RecruitmentActions", () => {
	...
	describe("recruitUnit", () => {
		...
		it("upgrades existing unit instead of adding duplicate", () => {
			const existing = makeUnit("existing", "unit_a", [0, 0], { rank: 1, power: 10, maxLife: 100, life: 80 });
		...
		it("returns original session for unknown card", () => {
			const session = makeSession();
			const result = RecruitmentActions.recruitUnit(session, "nonexistent_card", null);
