...
function applyTimeoutDamage(
  ...
  const tickCount =
    Math.floor(timeSinceTimeoutStarted / TIMEOUT_DAMAGE_INTERVAL_MS) + 1;
  const currentDamage = Math.floor(
    TIMEOUT_BASE_DAMAGE * Math.pow(TIMEOUT_GROWTH_RATE, tickCount - 1),
  );
  ...
  env.deferredEvents.push({
    ...
    execute: (env) => {
      const playerCore = Card.getBattleCore(env.combatState)(
        Constants.FORCE_ID_PLAYER,
      );
      const cpuCore = Card.getBattleCore(env.combatState)(
...
export function stopTimeoutDamageSystem(
  timeoutState: TimeoutSystemState,
): TimeoutSystemState {
  return {
    ...timeoutState,
...
export function onTimeoutDamageCombatEnd(
  timeoutState: TimeoutSystemState,
): TimeoutSystemState {
  return timeoutState.isActive
...
export function getTimeoutDamageConfig(timeoutState: TimeoutSystemState) {
  return {
    ...
    stormState: {
      stormStarted:
        timeoutState.combatElapsedTime >= Constants.TIMEOUT_DAMAGE_START_TIME,
    },
  };
}
