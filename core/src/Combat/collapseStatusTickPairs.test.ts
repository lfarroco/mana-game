...
describe("collapseStatusTickPairs", () => {
  ...
  it("merges a poison tick with a regen tick without carrying shield data", () => {
    // Poison pierces shield, so both halves of the pair only move life.
    const logs = [
      poisonTick(1000, "player", 20, 500, -20),
    ...
    expect(merged).not.toHaveProperty("newShield");
    expect(merged).not.toHaveProperty("shieldDelta");
  });
});
