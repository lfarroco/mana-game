...
const EFFECT_REGISTRY: Record<string, EffectFactory> = {
	...
	sloweffect: (scene) => {
		scene.time.addEvent({
			delay: 1500,
			callback: () => {
				effects.slowEffect({ x: 150, y: 350 }, { duration: 1000, intensity: 1.0 });
				effects.slowEffect({ x: 300, y: 500 }, { duration: 1000, intensity: 1.5 });
				effects.slowEffect({ x: 450, y: 350 }, { duration: 1000, intensity: 0.7 });
			},
			repeat: -1,
		});
	},
...
export class DebugScene extends Phaser.Scene {
	...
	private createEffectButtons() {
		...
		const title = this.add
			.text(constants.MIDDLE_SCREEN_X, 60, "DEBUG EFFECTS", {
				fontSize: "48px",
				color: "#ffffff",
				stroke: "#000000",
				strokeThickness: 6,
