...
import { createComponent, type ManaMsg, handleManaMsg, type Element } from '../index';

import { createButton, createButtonGroup, destroyButton } from './manabutton';



// Define your custom message types that extend ManaMsg

type GameMsg = ManaMsg | {
...
export const setupButtonExample = (scene: Phaser.Scene) => {
	...
	const render = createComponent<GameMsg>(scene, (msg, state) => {
		...
		// Handle ManaMsg actions first (tweens, redraws, etc.)

		// This processes button hover animations and other built-in actions

		if (msg.type.startsWith('@mana/')) {

			handleManaMsg(msg as ManaMsg, state as any);
		...
		// (ManaMsg actions are already handled above, so only custom msgs will reach this)
	...
	render(allComponents as Element<GameMsg | ManaMsg>[]);
...
export const setupDynamicButtonExample = (scene: Phaser.Scene) => {
	...
	const render = createComponent<GameMsg>(scene, (msg, state) => {
		...
		// Handle custom messages using type guard
