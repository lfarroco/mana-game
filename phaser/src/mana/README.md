...
- **React-Style Hooks**: Function components with `useState`, `useEffect`, `useDispatch`, and more
- **Ergonomic Events**: Interaction handlers can return a single message, an array, or nothing
...
Mana ships with a hook-based runtime that mirrors React's developer experience while still rendering through Phaser. Define function components that use hooks and render them via `createManaApp`.
...
import type { Element, ManaFC } from './mana';

type Msg =
  | { type: 'INCREMENT' }
  | { type: 'LOG'; value: number };

const Counter: ManaFC<{ label: string }, Msg> = ({ label }) => {
  ...
  return [
    text('counter', 10, 10, `${label}: ${count}`) as Element<Msg>,
  ];
};

const app = createManaApp<Msg, { label: string }>(scene, Counter, {
  update: (msg, state) => {
...
app.render({ label: 'Score' });

// Update props later
app.render({ label: 'Streak' });
```

### Nested components

Compose components via the `useComponent` hook to keep local state isolated and reusable:

```typescript
const Hud: ManaFC<{ score: number }, Msg> = ({ score }) => {
  const badge = useComponent(Badge, { score }, 'badge');
  return [
    container('hud', 0, 0, [
      ...badge,
      text('score', 20, 20, `Score: ${score}`),
    ]),
  ];
};
```
...
Event handlers like `onClick`, `onHover`, and `onHoverOut` now accept a single message, an array of messages, or nothing at all:

```typescript
const button = image('play', 200, 200, 'play-button', () => {
  playSound('click');
  return { type: 'START_GAME' };
});

const toggle = rectangle('toggle', 300, 200, 80, 40, 0xffffff, () => {
  setVisibility((visible) => !visible);
  // No messages dispatched – side-effects only
});
```

Under the hood, Mana normalizes the return values and dispatches them through the current component state.
