...
const getSkillDescription = (skill: Skill): string => {
	...
	switch (effect.id) {
		case 'damage':
			return `[color=#ff6b6b]Damage[/color] [color=#ffd93d]${effect.amount}[/color]\n[color=#c0c0c0]Deals damage to enemy forces[/color]`;
		case 'heal':
			return `[color=#51cf66]Heal[/color] [color=#ffd93d]${effect.amount}[/color]\n[color=#c0c0c0]Restores morale to allied forces[/color]`;
		case 'shield':
			return `[color=#74c0fc]Shield[/color] [color=#ffd93d]${effect.amount}[/color]\n[color=#c0c0c0]Grants shield protection[/color]`;
		case 'ally_damage_power_boost':
			return `[color=#ff8cc8]Battle Fury[/color] [color=#ffd93d]+${effect.powerBonus}[/color]\n[color=#c0c0c0]Allied units gain power when using damage effects[/color]`;
		default:
