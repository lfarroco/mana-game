...
function saveStore(store: GhostStoreData) {
	try {
		...
		Logger.warn("GhostStore", "[GhostStore] Failed to persist ghosts", err);
	}
}
...
function computeHash(units: GhostUnit[]): string {
	return units
		...
		.map(
			(u) =>
				`${u.cardId}@${u.x},${u.y}@${u.power ?? ""}@${u.effects?.length ?? 0}@${u.reactions?.length ?? 0}`
		)
		.join("|");
}
...
export function pickRandomGhost(round: number): GhostEntry | null {
	...
	const idx = Math.floor(nextValue() * validList.length);
...
export function instantiateGhostUnits(state: State, entry: GhostEntry): Unit[] {
	return entry.units.map((g) => {
		...
		if (g.reactions)
			unit.reactions = g.reactions.map((r) => ({
				...r,
				effects: r.effects?.map((e) => ({ ...e })) || [],
			}));
		return unit;
