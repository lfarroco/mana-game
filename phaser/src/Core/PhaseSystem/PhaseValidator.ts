import { PhaseOption, PhaseType, SessionData } from "../Types";
import { PhaseTransitionContext, PhaseTransitionResult, ValidationResult } from "./types";
...
class PhaseValidator {
	...
	public validateTransition(from: PhaseType, to: PhaseType): ValidationResult {
		...
		if (!allowed || !allowed.includes(to)) {
			return {
				valid: false,
				errors: [`Invalid phase transition from '${from}' to '${to}'. Allowed: ${allowed?.join(', ') || 'none'}`]
			};
		}
	...
	public validateAction(context: PhaseTransitionContext): ValidationResult {
		...
		if (meta && meta.fromPhase && meta.fromPhase !== phase) {
			return {
				valid: false,
				errors: [`Action '${actionId}' is valid for phase '${meta.fromPhase}', but current phase is '${phase}'.`]
			};
		}
	 ...
	 */
	public validateResult(previousSession: SessionData, result: PhaseTransitionResult): ValidationResult {
