...
export const create = () =>
	new Promise<void>((resolve) => {
				...
				for (let i = 0; i < pageCards.length; i++) {
					...
					if (!isUnlocked) {
						...
						chara.on(Phaser.Input.Events.POINTER_OVER, () => {
							const { title, description: normalDescription } =
								createDescription.createDescription(chara);
		...
		const pageIndicator = env.scene.add
			.text(0, PANEL_HEIGHT / 2 - 120, "1 / 1", { fontSize: "24px", color: "#ffffff" })
			.setOrigin(0.5);
