...
export function issueSkillCommand(
  ...
  if (skill.targets === "ally") {
    ...
    emit(signals.SELECT_SKILL_TARGET_DONE, unit.id, skill.id, asVec2(tile), allyInTile.id);
    ...
    emit(signals.SELECT_SKILL_TARGET_DONE, unit.id, skill.id, asVec2(tile), enemy.id);
