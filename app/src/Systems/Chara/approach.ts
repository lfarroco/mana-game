...
export async function approach(
	...
	const distance = distanceBetween(unit.position)(closestUnit.position);
	...
	if (distance > range) {
		...
		await walk(scene, unit, pathTo, (position: Vec2) => {
			const distance = distanceBetween(position)(closestUnit.position);
			return distance <= range;
		});
	}

	const finalDistance = distanceBetween(unit.position)(closestUnit.position);
