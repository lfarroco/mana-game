...
- [x] Added visible `Username` and `Email` labels above the Arena account form fields in `phaser/src/Client/Scenes/ArenaLobby/ArenaLoginScene.ts` so the guest conversion and account management screens show explicit field labels. (Copilot, 2026-05-12)
...
- [x] Refreshed the Arena leaderboard modal in `phaser/src/Client/Scenes/ArenaLobby/ArenaLobbyScene.ts` to use the shared modal styling and a formatted leaderboard table with headers, highlighted top ranks, improved empty/error states, and persistent pagination controls; updated `ArenaLobbyScene.test.ts` to cover the revised modal wiring. (Copilot, 2026-05-12)
...
- [x] Added a decoupled headless HTTP agent server CLI in `phaser/server/index.ts` and `phaser/server/agentGameServer.ts`, exposing Core-backed endpoints for creating games, querying board/choices/card details, rearranging the board, making choices, and retrieving the replay manifest; covered with an HTTP integration test in `phaser/server/AgentGameServer.test.ts` and documented in `phaser/server/README.md` (Copilot, 2026-03-24)
- [x] Added a pure server-side LLM play service in `phaser/src/Core/LlmPlayerService.ts` that exposes board viewing, board arrangement, card inspection, choice execution, and in-memory replay-manifest recording for headless AI players; covered with focused Jest tests in `phaser/src/Core/LlmPlayerService.test.ts` and re-exported from `phaser/src/Core/GameLogic.ts` (Copilot, 2026-03-24)
- [x] Dead-code cleanup slice from roadmap: removed orphaned module `phaser/src/Systems/MatchResultSystem.ts` (no imports/usages), removed duplicate `initializeVisualizer()` call from `phaser/src/Client/Scenes/Battleground/BattlegroundScene.ts`, and validated no phase-system regressions with `src/Core/PhaseSystem/handlers/PhaseHandlers.test.ts` (38/38 passing) (Copilot, 2026-03-24)
...
- [x] Centralized crystal selection scene UI layout and animation constants in `phaser/src/Client/Scenes/CrystalSelection/CrystalSelectionScene.ts` (title/font sizes, button widths, pagination styling, display dimensions, animation timings) (Copilot, 2026-03-20)
- [x] Centralized arena login scene layout and styling constants in `phaser/src/Client/Scenes/ArenaLobby/ArenaLoginScene.ts` (title position/font, form dimensions, button offsets, modal styling) (Copilot, 2026-03-20)
- [x] Centralized login modal panel and styling constants in `phaser/src/Client/Scenes/ArenaLobby/LoginModal.ts` (panel dimensions, colors, strokes, title font, button offsets) (Copilot, 2026-03-20)
...
- [x] Centralized arena lobby scene layout and styling constants in `phaser/src/Client/Scenes/ArenaLobby/ArenaLobbyScene.ts` (title/profile/rating font sizes, positioning, button offsets) (Copilot, 2026-03-20)
