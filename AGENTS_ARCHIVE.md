...
- [x] **P1 Step 4: `on_over_heal` evaluated at hit time instead of cast time** — Moved the overheal condition check from cast time (200ms before the heal lands) into the deferred `execute` closure, against the core's life at hit time. Renamed `hasOnOverHealReaction` to `willOverheal` (it checks whether the heal would exceed maxLife, not whether a reaction exists). Removed the stale `sourceForce` cast-time variable. (Cline, 2026-07-25)

- [x] **P1 Step 8: `sacrifice_effect_orb` no longer a silent no-op** — Added `applySacrificeOrb` in `OrbAndCoreUpgrades.ts`: removes a random effect or reaction from the target unit (using seeded RNG) and grants +10 flat power (matching the combat `sacrificeEffect` bonus). Wired into the `applyOrb` dispatch chain. Added `SACRIFICE_POWER_INCREASE` constant in `OrbConstants.ts`. Added 5 unit tests covering effect removal, reaction removal, no-op fallback, seed advancement, and no-advance when no effects/reactions exist. All 401 core tests pass. (Cline, 2026-07-25)
